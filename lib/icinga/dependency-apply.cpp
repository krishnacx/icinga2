...
bool Dependency::EvaluateApplyRuleInstance(const Checkable::Ptr& checkable, const String& name, ScriptFrame& frame, const ApplyRule::Ptr& rule)
{
...
bool Dependency::EvaluateApplyRule(const Checkable::Ptr& checkable, const ApplyRule::Ptr& rule)
{
	DebugInfo di = rule->GetDebugInfo();
...
void Dependency::EvaluateApplyRules(const Host::Ptr& host)
{
	...
	for (const ApplyRule::Ptr& rule : ApplyRule::GetRules("Dependency")) {
